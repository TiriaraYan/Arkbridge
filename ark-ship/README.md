...
给 Claude Code 用的结构化 ship（提交前检查）流程。

在你的项目里输入 `/ship`，Claude 会自动走完 **8 个步骤**：盘点改动 → 识别管线 → 同步文档 → 安全扫描 → 语法检查 → 干净提交 → 清理 → 自省。每一步都有明确的"做什么"和"为什么"——不是靠纪律记住该做什么，而是让流程替你记住。
...
这通过一个注册表文件 `.claude/pipelines.json` 实现——它记录：
